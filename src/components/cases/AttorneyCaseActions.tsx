...
export function AttorneyCaseActions({ caseId, caseNumber, patientName, currentFlag, providerId }: Props) {
  ...
  const dropCase = useMutation({
    mutationFn: async () => {
      if (terminationFile) {
        ...
        await supabase.from('documents').insert({
          case_id: caseId, file_name: terminationFile.name, storage_path: path,
          document_type: 'Termination Letter', uploader_id: user?.id,
      ...
      await supabase.from('case_updates').insert({
        case_id: caseId, author_id: user?.id,
      ...
      await supabase.from('case_timelines').insert({
        case_id: caseId, event_date: new Date().toISOString().split('T')[0],
        event_type: 'Case Closed', event_title: `Case dropped by attorney — ${profile?.full_name}`,
        event_detail: dropReason || null, visible_to: ['admin', 'care_manager', 'attorney', 'provider'],
      });
      try {
    ...
    onSuccess: () => {
      ...
      setShowDropConfirm(false); setDropReason(''); setTerminationFile(null);
  ...
  const changeStatus = useMutation({
    mutationFn: async (status: AttorneyCaseStatus) => {
      if (status === 'Dropped') { setShowDropConfirm(true); return; }
  ...
  const uploadTerminationLetter = useMutation({
    mutationFn: async () => {
      ...
      if (uploadError) throw uploadError;
      await supabase.from('documents').insert({
        case_id: caseId, file_name: uploadFile.name, storage_path: path,
        document_type: 'Termination Letter', uploader_id: user?.id,
        visible_to: ['admin', 'care_manager', 'attorney'],
      });
    },
  ...
  return (
    <>
      ...
      <div className="flex items-center gap-2">
        <Select value={currentStatus} onValueChange={(v) => changeStatus.mutate(v as AttorneyCaseStatus)}>
          <SelectTrigger className="h-9"><SelectValue /></SelectTrigger>
          <SelectContent>
            {ATTORNEY_CASE_STATUSES.map(s => (
              <SelectItem key={s} value={s}>
                <span className={s === 'Dropped' ? 'text-destructive font-medium' : ''}>{s}</span>
              </SelectItem>
            ))}
          </SelectContent>
        </Select>
        ...
        <DialogContent>
          ...
          <div className="space-y-4">
            <div className="space-y-2">
              ...
              <Textarea value={dropReason} onChange={e => setDropReason(e.target.value)} placeholder="e.g. Client non-responsive, liability issues..." rows={3} />
            </div>
            <div className="space-y-2">
