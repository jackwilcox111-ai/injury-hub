...
import { Search, UserPlus, CheckSquare, MapPin, Phone, Globe, Building2 } from 'lucide-react';
...
export function TaskDetailDialog({ open, onOpenChange, task, staff, onUpdate }: TaskDetailDialogProps) {
  ...
  const specialty = useMemo(() => extractSpecialty(task?.title || ''), [task?.title]);
  const isReferralTask = !!specialty;

  const { data: providers, isLoading: loadingProviders } = useQuery({
    queryKey: ['task-providers', specialty],
    enabled: open && isReferralTask,
    queryFn: async () => {
      let q = supabase.from('providers').select('id, name, specialty, phone, address_city, address_state, accepting_patients, languages_spoken, rating')
        .eq('status', 'Active');
      if (specialty) {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className={isReferralTask ? "sm:max-w-2xl max-h-[85vh] overflow-y-auto" : "sm:max-w-md"}>
        ...
        <div className="space-y-4">
          ...
          {/* Provider Assignment Section (only for referral tasks) */}
          {isReferralTask && task.status !== 'Complete' && (
            <div className="border-t border-border pt-4 space-y-3">
              <div className="flex items-center justify-between">
                <h3 className="text-sm font-semibold text-foreground flex items-center gap-1.5">
                  <UserPlus className="w-4 h-4 text-primary" />
                  Assign {specialty} Provider
                </h3>
              ...
              <div className="max-h-64 overflow-y-auto space-y-1.5 pr-1">
                ...
                {!loadingProviders && filteredProviders.length === 0 && (
                  <p className="text-xs text-muted-foreground text-center py-4">No matching {specialty} providers found</p>
                )}
