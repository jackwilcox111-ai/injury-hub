...
export function ProviderMapView({ providers, activeId, onSelectProvider }: Props) {
    ...
    mappable.forEach(p => {
      ...
      const icon = L.divIcon({
        ...
        html: `<div style="
          ...
          ${isActive ? `box-shadow: 0 0 0 3px ${color}, 0 4px 12px rgba(0,0,0,0.4);` : ''}
      ...
      const marker = L.marker([p.latitude!, p.longitude!], { icon })
        .bindPopup(`
          <div style="min-width:180px">
            ...
            ${p.specialty ? `<span style="font-size:11px;color:${color}">${p.specialty}</span><br/>` : ''}
            <span style="font-size:11px;color:${accepting ? '#16a34a' : '#6b7280'}">${accepting ? '✓ Accepting Patients' : '✗ Not Accepting'}</span><br/>
            ${address ? `<span style="font-size:11px">${address}</span><br/>` : ''}
            ${p.phone ? `<span style="font-size:11px">${p.phone}</span>` : ''}
          </div>
        `)
